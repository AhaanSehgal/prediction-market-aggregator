'use client';

import { ConnectionStatus } from '@/components/market/ConnectionStatus';
import { useOrderBookStore } from '@/stores/orderbook-store';
import { DEFAULT_MARKET } from '@/domain/market/constants';
import { formatCents } from '@/lib/utils';

const NAV_LINKS = ['Discover', 'Portfolio', 'Wallet Tracker', 'Leaderboard', 'Watchlist', 'Referrals'];

export function AppShell({ children }: { children: React.ReactNode }) {
  const mergedBook = useOrderBookStore((s) => s.mergedBook);
  const yesPrice = mergedBook.midpoint;
  const noPrice = yesPrice !== null ? 1 - yesPrice : null;

  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background">
      {/* Primary nav bar */}
      <header className="flex items-center justify-between px-4 h-11 border-b border-border bg-surface shrink-0">
        <div className="flex items-center gap-6">
          {/* Logo */}
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-[15px]">🔥</span>
            <span className="text-sm font-semibold tracking-tight">
              <span className="text-foreground">PM</span>
              <span className="text-muted-light ml-0.5 text-xs font-normal">AGG</span>
            </span>
          </div>

          {/* Nav links */}
          <nav className="hidden lg:flex items-center gap-1">
            {NAV_LINKS.map((link) => (
              <button
                key={link}
                className="px-2.5 py-1 text-[13px] text-muted-light hover:text-foreground transition-colors rounded-md hover:bg-surface-2"
              >
                {link}
              </button>
            ))}
          </nav>
        </div>

        <div className="flex items-center gap-3">
          {/* Search */}
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-surface-2 border border-border rounded-md w-48">
            <svg className="w-3.5 h-3.5 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <span className="text-[12px] text-muted">Search</span>
            <span className="ml-auto text-[10px] text-muted border border-border-light rounded px-1 py-0.5 font-mono">K</span>
          </div>

          <ConnectionStatus />

          {/* Sign In */}
          <button className="px-3 py-1 text-[13px] text-foreground bg-surface-2 border border-border-light rounded-md hover:bg-surface-3 transition-colors">
            Sign In
          </button>
        </div>
      </header>

      {/* Market sub-header */}
      <div className="flex items-center gap-3 px-4 h-10 border-b border-border bg-surface shrink-0 overflow-x-auto">
        {/* Star + Market title */}
        <div className="flex items-center gap-2 shrink-0">
          <button className="text-muted hover:text-foreground transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
          </button>
          <span className="text-[13px] text-foreground font-medium truncate max-w-xs">
            {DEFAULT_MARKET.title}
          </span>
        </div>

        {/* YES/NO pills */}
        {yesPrice !== null && noPrice !== null && (
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="inline-flex items-center px-3 py-1 text-[12px] font-mono font-semibold bg-bid/10 text-bid border border-bid/25 rounded">
              YES {formatCents(yesPrice)}
            </span>
            <span className="inline-flex items-center px-3 py-1 text-[12px] font-mono font-semibold bg-ask/10 text-ask border border-ask/25 rounded">
              NO {formatCents(noPrice)}
            </span>
          </div>
        )}

        {/* Divider */}
        <div className="w-px h-5 bg-border shrink-0" />

        {/* Market stats */}
        <div className="hidden lg:flex items-center gap-5 text-[11px] font-mono shrink-0">
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">Expires</span>
            <span className="text-muted-light">Nov 5, 2028</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">24h Change</span>
            <span className="text-bid">+1.2¢</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">24h Volume</span>
            <span className="text-muted-light">37.8K</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">Total Volume</span>
            <span className="text-muted-light">5.6M</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">O.I.</span>
            <span className="text-muted-light">8.7M</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-muted text-[10px]">Volatility</span>
            <span className="text-muted-light">0.42%</span>
          </div>
        </div>
      </div>

      {/* Main content */}
      <main className="flex-1 overflow-hidden">{children}</main>
    </div>
  );
}
