...
import { useBookForQuote } from '@/hooks/useBookForQuote';
...
export function QuotePanel() {
  ...
  const { quote, isNo, outcomeLabel, currentPrice } = useBookForQuote(inputAmount);
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <OutcomeRow isNo={isNo} outcomeLabel={outcomeLabel} currentPrice={currentPrice} />
      ...
      <QuickAmounts isBuy={isBuy} onSelect={handleQuickAmount} />

      {quote && quote.totalShares > 0 ? (
        <>
          <QuoteDetails quote={quote} isBuy={isBuy} />
          {quote.venueSummaries.length > 0 && <FillSplit quote={quote} />}
        </>
      ) : inputAmount > 0 ? (
...
function QuickAmounts({ isBuy, onSelect }: { isBuy: boolean; onSelect: (amt: number) => void }) {
  return (
...
function QuoteDetails({ quote, isBuy }: { quote: NonNullable<ReturnType<typeof useBookForQuote>['quote']>; isBuy: boolean }) {
  return (
...
function FillSplit({ quote }: { quote: NonNullable<ReturnType<typeof useBookForQuote>['quote']> }) {
  return (
    <div className="px-3 py-2.5 border-b border-border shrink-0">
...
function PayoutSummary({
  ...
  quote: ReturnType<typeof useBookForQuote>['quote'];
  isBuy: boolean;
