...
**Venue-agnostic merge layer.** Each venue normalizes its raw API data into a `NormalizedOrderBook`. The merge algorithm is venue-agnostic - adding a third venue (e.g., a new prediction market) would only require a new normalizer and socket adapter.
...
**Venue-specific tick sizes.** Kalshi uses cent-level pricing (minimum 1¢ ticks), so when the order book is filtered to Kalshi-only, tick options are restricted to 1¢/2¢. For NO outcome with "All" venues, tick defaults to 1¢ since sub-cent levels are sparse after flipping.
...
- **Tests**: Unit tests for the merge algorithm and quote engine (these are pure functions, easily testable), integration tests for WebSocket reconnection behavior.
